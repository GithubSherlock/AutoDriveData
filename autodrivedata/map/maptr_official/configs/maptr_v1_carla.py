...
- v1 主分支**只有上线数据集** `CustomNuScenesLocalMapDataset`:它的 `VectorizedLocalMap`
  在 `__init__` 构造 4 张 `NuScenesMap`(需 `maps/expansion/*.json`,`autodrivedata/map/prepare_official_dataset.py`
  已生成空桩),训练时按位姿查地图库并做全局→ego 变换。故这里用项目侧子类
  `CarlaNuScenesLocalMapDataset`(`autodrivedata/map/maptr_official/bridge.py`)把 `self.vector_map` 换成
...
所以仓库根必须在 PYTHONPATH 上):
    ...
    cd /root/autodl-tmp/Documents/Projects/AutoDriveData/hdMapGitHub/MapTR && \
      ...
      python tools/train.py \
        /root/autodl-tmp/Documents/Projects/AutoDriveData/autodrivedata/map/maptr_official/configs/maptr_v1_carla.py
"""
...
# 项目侧数据源桥(v1 专用;见 autodrivedata/map/maptr_official/bridge.py)。allow_failed_imports=False:
# 桥没接上就必须**立刻**炸,不能退化成"官方数据集去查空地图"这种静默错误。
custom_imports = dict(imports=["autodrivedata.map.maptr_official.bridge"], allow_failed_imports=False)
