...
#
# 用法:
#   bash autodrivedata/map/maptr_official/run_official.sh train maptrv2 [--resume] [--bg]  # --resume=有 latest 就接着训
#   bash autodrivedata/map/maptr_official/run_official.sh chain [--bg]   # 三线串行(v1 → mapqr → maptrv2),失败即停
#   bash autodrivedata/map/maptr_official/run_official.sh test  maptrv2 [ckpt|best]        # 缺省=work_dir/latest.pth
#   bash autodrivedata/map/maptr_official/run_official.sh list
...
[ -n "$IMPL" ] || usage 2
REPO="$(impl_repo "$IMPL")"
CFG="$PROJ/autodrivedata/map/maptr_official/configs/${IMPL}_carla.py"
WORK_DIR="$(work_dir_of "$IMPL")"
[ -f "$CFG" ] || { echo "缺配置:$CFG" >&2; exit 2; }
[ -x "$PY" ] || { echo "缺环境:$ENV(先跑 autodrivedata/map/maptr_official/setup_maptr_official.sh)" >&2; exit 2; }
