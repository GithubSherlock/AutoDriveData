"""`selfcheck` 手算锚点单测(纯 numpy,不碰 CARLA)。
...
ROT = sc.cam_to_world_rot(CAM_ROT_FWD)
...
def _samples(e: np.ndarray, g: np.ndarray, z0: float = 20.0) -> sc.DepthSamples:
...
class TestPlaneRay:
    def test_head_on_hit(self):
        ...
        pl = sc.Plane(np.array([1.0, 0.0, 0.0]), 10.0)
    ...
    def test_tilted_ray_ray_length_differs_from_axis_depth(self):
        ...
        (`sec θ`),图像角落可达 +44%。
        """
        pl = sc.Plane(np.array([0.0, 0.0, 1.0]), -1.0)  # 地面 z = −1
...
class TestFitPlane:
    def test_recovers_known_plane(self):
        ...
        fit = sc.fit_plane(np.column_stack([pts, z]))
    ...
    def test_too_few_points_is_none(self):
        assert sc.fit_plane(np.zeros((sc.MIN_PLANE_PTS - 1, 3))) is None

    def test_collinear_points_is_none(self):
        t = np.linspace(0, 1, 40)[:, None]
        assert sc.fit_plane(np.hstack([t, np.zeros_like(t), np.zeros_like(t)])) is None
    ...
    def test_noisy_plane_residual_is_reported(self):
        ...
        assert sc.plane_is_usable(fit[1])
        assert not sc.plane_is_usable(0.5)
    ...
    def test_local_planes_on_curved_surface(self):
        ...
        pts = u * 20.0
        normals, rms = sc.fit_local_planes(pts, radius=3.0)
        ok = np.isfinite(rms)
        assert ok.sum() > 5000
...
class TestProjectionInverse:
    def test_ray_and_project_are_inverse(self):
        ...
        back, z_axis = sc.project_world(pts_world, CAM_LOC, CAM_ROT_FWD, K)
    ...
    def test_camera_frame_is_not_world_frame(self):
        ...
        rays = sc.cam_rays(K, np.array([[K.cx, K.cy]]))
    ...
    def test_ray_is_unit_length(self):
        ...
        assert np.allclose(np.linalg.norm(sc.cam_rays(K, uv), axis=1), 1.0)

    def test_convention_does_not_enter_the_ray(self):
        """`cam_rays` 吃的是**图像坐标**,与像素约定无关(约定只在采样时出现)。"""
        uv = np.array([[300.0, 200.0]])
        assert np.array_equal(sc.cam_rays(K, uv), sc.cam_rays(K, uv))
...
class TestBackproject:
    def test_plane_depth_roundtrip(self):
        ...
        z = 30.0
        depth = np.full((H, W), z)
        pts = sc.backproject_depth(uv, depth, K, CONVENTION_CENTER)
        back, z_axis = sc.project_world(pts, CAM_LOC, CAM_ROT_FWD, K)
    ...
    def test_convention_shift_moves_sampling_by_half_pixel(self):
        ...
        def z_of(u: float, conv: str) -> float:
            pts = sc.backproject_depth(np.array([[u, 100.0]]), depth, K, conv)
            return float(sc.project_world(pts, CAM_LOC, CAM_ROT_FWD, K)[1][0])
...
class TestLocalRange:
    ...
    def test_zero_radius_is_zero(self):
        ...
        assert np.allclose(sc.local_range(img, 0), 0.0)
...
class TestAxisEstimation:
    def test_recovers_injected_delta(self):
        ...
        fit = sc.estimate_axis_delta(_samples(e, g), axis=0)
    ...
    def test_sigma_shrinks_with_more_samples(self):
        ...
        def sigma(n: int) -> float:
            ...
            return sc.estimate_axis_delta(_samples(e, g), axis=0).sigma_delta_px
    ...
    def test_v_axis_is_independent(self):
        ...
        s = sc.DepthSamples(np.zeros((n, 2)), np.zeros(n), e, g)
        fu, fv = sc.estimate_delta_uv(s)
    ...
    def test_too_few_samples(self):
        fit = sc.estimate_axis_delta(_samples(np.array([1.0, 2.0]), np.array([0.1, 0.2])), axis=0)
    ...
    def test_sweep_profile_minimum_matches_regression(self):
        ...
        prof = sc.sweep_profile(s, 0, deltas)
        assert deltas[int(np.argmin(prof))] == pytest.approx(sc.estimate_axis_delta(s, 0).delta_px, abs=0.05)

    def test_mad_sigma_is_robust_to_outliers(self):
        rng = np.random.default_rng(13)
        x = rng.normal(scale=1.0, size=2000)
        x[:50] += 500.0
        assert 0.8 < sc.mad_sigma(x) < 1.2


class TestDepthSemantics:
    def test_axis_model_is_flat_and_range_model_is_sec(self):
        theta = np.radians([0.0, 30.0, 46.2])
        z_model, r_model = sc.depth_model_ratio(theta)
    ...
    def test_max_off_axis_angle_anchor(self):
        ...
        assert math.degrees(sc.max_off_axis_angle(K)) == pytest.approx(46.27, abs=0.05)

    def test_off_axis_angle_zero_on_axis(self):
        assert sc.off_axis_angle(np.array([[K.cx, K.cy]]), K)[0] == pytest.approx(0.0, abs=1e-12)
...
class TestRadialProfile:
    def test_ideal_pinhole_has_zero_slope(self):
        ...
        s = sc.DepthSamples(uv, np.zeros(n), e, np.zeros((n, 2)))
        _, _, counts, slope = sc.radial_error_profile(s, K)
    ...
    def test_radial_bias_shows_up_as_slope(self):
        ...
        s = sc.DepthSamples(uv, np.zeros(n), e, np.zeros((n, 2)))
        _, _, _, slope = sc.radial_error_profile(s, K)
        assert slope == pytest.approx(1e-4, rel=0.1)

    def test_empty_samples(self):
        empty = sc.DepthSamples(np.zeros((0, 2)), np.zeros(0), np.zeros(0), np.zeros((0, 2)))
        _, _, counts, slope = sc.radial_error_profile(empty, K)
...
class TestAxisTarget:
    def test_midpoints_recover_centre(self):
        ...
        _, mids = sc.mask_row_midpoints(_strip_mask(621.0, width=20.0))
        assert mids.size > 30
        assert np.allclose(mids, 620.5, atol=1e-9)

    def test_midline_deviation_zero_for_symmetric(self):
        _, mids = sc.mask_row_midpoints(_strip_mask(621.0, width=20.0))
        assert sc.midline_deviation(mids) == pytest.approx(0.0, abs=1e-9)
    ...
    def test_midline_deviation_flags_one_sided_tilt(self):
        ...
        mask[:20, :618] = False
        _, mids = sc.mask_row_midpoints(mask, trim=0.0)
        assert sc.midline_deviation(mids) == pytest.approx(3.5, abs=1e-9)

    def test_mirror_asymmetry_zero_for_symmetric(self):
        assert sc.mirror_asymmetry(_strip_mask(621.0, width=20.0), 620.5) == pytest.approx(0.0, abs=1e-9)

    def test_mirror_asymmetry_flags_offset_axis(self):
        # 条带半宽 10 px;镜像面偏 4 px ⇒ 左右半宽差 8 px
        assert sc.mirror_asymmetry(_strip_mask(621.0, width=20.0), 616.5) == pytest.approx(8.0, abs=1e-9)
    ...
    def test_mirror_asymmetry_is_about_a_fixed_axis_not_the_midline(self):
        ...
        assert sc.mirror_asymmetry(mask, 620.5) == pytest.approx(0.0, abs=1e-9)
        # 上半半宽 (611..630 绕 616.5) = 5.5 vs 13.5 ⇒ 8.0
        assert sc.mirror_asymmetry(mask, 616.5) == pytest.approx(8.0, abs=1e-9)
    ...
    def test_prop_regression_recovers_cx(self, shift):
        ...
        cx_est, slope, resid = sc.prop_axis_regression(offsets, centres, shift)
    ...
    def test_prop_regression_requires_two_groups(self):
        with pytest.raises(ValueError):
            sc.prop_axis_regression(np.array([0.0]), np.array([621.0]), 0.5)
    ...
    def test_prop_regression_residual_flags_moved_prop(self):
        ...
        centres[3] += 0.8  # 该组被挪了
        _, _, resid = sc.prop_axis_regression(offsets, centres, 0.5)
...
class TestCollectSamples:
    ...
    def test_occlusion_filter_drops_discontinuity(self):
        ...
        pl_o = np.array([25.0])
        s_no = sc.collect_samples(pts, pl_n, pl_o, CAM_LOC, CAM_ROT_FWD, K, depth)
        assert len(s_no) == 1
        s_oc = sc.collect_samples(pts, pl_n, pl_o, CAM_LOC, CAM_ROT_FWD, K, depth, occlusion_radius_px=8)
        assert len(s_oc) == 0
...
class TestVisibilityIsOneSided:
    ...
    def _one(self, z_pred: float, z_rend: float, **kw) -> int:
        ...
        return len(sc.collect_samples(pts, n, np.array([z_pred]), CAM_LOC, CAM_ROT_FWD, K, depth, **kw))
    ...
    def test_nearer_neighbour_in_window_does_not_kill(self):
        ...
        pts = _world(uv, 25.0)
        n = np.array([[1.0, 0.0, 0.0]])
        s = sc.collect_samples(pts, n, np.array([25.0]), CAM_LOC, CAM_ROT_FWD, K, depth)
        assert len(s) == 1
        # 显式开窗口极差口径则被剔 —— 保留这条对照,证明"关掉它"是刻意的
        s_win = sc.collect_samples(
