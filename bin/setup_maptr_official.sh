...
#
# 用法:bash bin/setup_maptr_official.sh [system|conda|pip|ckpt|build|verify|all]
...
HEAD = "# 由 AutoDriveData 的 bin/setup_maptr_official.sh 生成的 av2 垫片,不是真 SDK。\n"
    ...
    class _Unused:
        def __init__(self, *args, **kwargs):
            raise NotImplementedError(
                f"{name} 是 av2 垫片占位(见 bin/setup_maptr_official.sh 的 install_av2_stub)。"
...
由 AutoDriveData 的 `bin/setup_maptr_official.sh` 生成,不是 numba 自带文件。
...
重装 numba 会覆盖掉本文件 → 重跑 `bash bin/setup_maptr_official.sh pip` 即可恢复。
"""
