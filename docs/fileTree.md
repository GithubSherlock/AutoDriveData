...
| 能力目录 | 规模 | 一句话 | 层约束 |
|---|---|---|---|
| `sim/` | 23 | CARLA 仿真交互层 + 全部采集器 + 平台探针 | 许 carla |
| `calib/` | 14 | 标定原语 / rig 表 / 自证探针 / 实时监看 / 配置图 | 许 carla,禁 torch |
| `map/` | 29 | 地图矢量 + MapTR(含 `maptr/`、`maptr_official/`) | 不设限 |
| `slam/` | 11 | 两段式激光 SLAM + 精度评估 + C++ 对拍 | **纯值** |
...
### `map/maptr/` — MapTR 参考自实现(7)

原顶层 `maptr_impl/`。torch 栈,禁 carla。
