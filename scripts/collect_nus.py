...
from carla_common import LIDAR_ATTRS, SENSOR_OFFSET, loc, rad, spawn_ego, spawn_npcs, sync_mode
...
CAM_ATTRS = {"image_size_x": "1600", "image_size_y": "900", "fov": "90"}  # nuScenes 分辨率
...
def _actor_to_annotation(a: carla.Actor, points_global: np.ndarray) -> dict | None:
    ...
    num_pts = count_points_in_box_nus(points_global, tuple(center_nus), size, yaw_nus)
    return {
        "category": category,
        "translation": tuple(float(v) for v in center_nus),
        "size": size,
        "yaw_nus": yaw_nus,
...
def main() -> None:
    ...
    print(f"[sensor] lidar {LIDAR_ATTRS['channels']}ch + 6 cameras {CAM_ATTRS['image_size_x']}x{CAM_ATTRS['image_size_y']}")
    ...
    calib_lidar = ((SENSOR_OFFSET.location.x, SENSOR_OFFSET.location.y, SENSOR_OFFSET.location.z), 0.0)
    calib_cameras = {
    ...
    try:
        for i in range(args.frames):
            ...
            ego_trans_nus = tuple(
                float(v) for v in g.carla_to_nus_global(
                    np.asarray([loc(ego_t_now)])
                )[0]
            )
            ego_yaw_nus = g.carla_yaw_to_nus_yaw(np.radians(ego_t_now.rotation.yaw))

            pts_global = points_sensor_to_global_nus(
                pts_nus[:, :3], ego_trans_nus, ego_yaw_nus, calib_lidar[0], calib_lidar[1]
            )
