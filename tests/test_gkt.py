...
from maptr_impl.gkt import GKT, BEVParams, _carla_rotation_torch, cam_world_pose, project_pts
...
def _cam_world_np(ego: np.ndarray, se: np.ndarray) -> tuple[np.ndarray, tuple[float, float, float]]:
    ...
    r_e = g.carla_rotation_matrix(tuple(np.radians(ego[3:6])))
    r_s = g.carla_rotation_matrix(tuple(np.radians(se[3:6])))
...
def _bev_margins_np(b: BEVParams, ego: np.ndarray, calibs: dict[str, dict]) -> np.ndarray:
    ...
    for se in calibs.values():
        ...
        r_e = g.carla_rotation_matrix(tuple(np.radians(ego[3:6])))
        r_s = g.carla_rotation_matrix(tuple(np.radians(s2e[3:6])))
        r_w2c = g.CARLA_TO_CAM @ (r_e @ r_s).T
        t_cam = ego[:3] + r_e @ s2e[:3]
        p_c = (pts @ r_e.T + ego[:3] - t_cam) @ r_w2c.T
        z = p_c[:, 2]
        u = CAM.fx * p_c[:, 0] / z + CAM.cx
        v = CAM.fy * p_c[:, 1] / z + CAM.cy
        # 像素中心级覆盖边界(与 GKT mask 同口径):u ∈ [0, W−1]
        m = np.minimum.reduce([u, CAM.width - 1 - u, v, CAM.height - 1 - v, z - 0.5])
...
def test_gkt_fusion_masks_match_oracle() -> None:
    ...
    feats = {name: torch.ones(2, 3, CAM.height, CAM.width, device=DEVICE) for name in calibs}
    bev, valid = gkt(feats, poses, calibs)
    ...
    for bi in range(2):
        margins = _bev_margins_np(b, poses[bi].cpu().numpy(), calibs)
        mask_np = margins > 0
